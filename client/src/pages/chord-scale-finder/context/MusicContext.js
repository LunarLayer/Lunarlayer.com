...
export default function MusicContextProvider({ children }) {
  ...
  const [fretboardDisplay, setFretboardDisplay] = useState("fretboard");
  ...
  const addString = ({position, notex}) => {
    switch (position) {
      case "top":
        strings.forEach((string, index) => {
          string.number++;
        });
        setStrings([{number: 1, note: notex}, ...strings]);
        break
      case "bottom":
        setStrings([...strings, {number: (strings.length + 1), note: notex}]);
        break
      default:
        break;
    }
  ...
  return (
    <MusicContext.Provider
      value={{
        fretboardDisplay, setFretboardDisplay,
        notes,
